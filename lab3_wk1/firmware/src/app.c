...
char ulValueToSend1 = 'a'; //formerly: const unsigned long u1ValueToSend1 = 100UL;
...
void APP_Tasks ( void )
{
   ...
   xQueueSend( xQueue, &ulValueToSend1, 0U );       //First thing to send
   
   /* Send to the queue - causing the queue receive task1 to unblock and
   toggle the LED.  0 is used as the block time so the sending operation
   will not block - it shouldn't need to block as the queue should always
   be empty at this point in the code. */
   //xQueueSend( xQueue, &ulValueToSend2, 0U );     //Second thing to send
